...
function HasBankAccountPermission(source, accountData, permission, stateId)
    ...
    elseif accountData.Type == 'organization' then
        ...
        for k, v in ipairs(pData) do
            ...
            -- owners bypass the BANK_ACCOUNT_* permission mapping entirely
            if plsr.Jobs.Permissions:IsOwner(source, v.job) then
                return true
            end

            local jp = json.decode(v.jobPermissions or "{}")
            if plsr.Jobs.Permissions:HasJob(source, v.job, workplace, false, false, false, jp[permission]) then
                return true
            end
        end
    end
