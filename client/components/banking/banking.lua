local config = load(LoadResourceFile(GetCurrentResourceName(), "config/shared.lua"))()

local withinBranchZone = false
local showingAction = false

function RunBankingStartup()
	AddBankingATMs()

	for k, v in pairs(config.Locations.branches) do
		if v.interactZone then
			plsr.Polyzone.Create:Box("banking-" .. k, v.interactZone.center, v.interactZone.length, v.interactZone.width, {
				heading = v.interactZone.heading,
				minZ = v.interactZone.minZ,
				maxZ = v.interactZone.maxZ,
			}, {
				bank_branch = k,
			})
		end
	end
end

AddEventHandler("Characters:Client:Spawn", function()
	for k, v in pairs(config.Locations.branches) do
		if v.blip then
			if v.special then
				plsr.Blips:Add("bank_" .. k, "Pacific Bank", v.blip, 272, 15, 0.8)
			else
				plsr.Blips:Add("bank_" .. k, "Bank", v.blip, 272, 69, 0.6)
			end
		end
	end
end)

AddEventHandler("Polyzone:Enter", function(id, point, insideZone, data)
	if data.bank_branch then
		withinBranchZone = data.bank_branch

        if not GlobalState[string.format("Fleeca:Disable:%s", data.bank_branch)] then
            plsr.Action:Show("banking", "{keybind}primary_action{/keybind} Open Bank")
        else
            plsr.Action:Show("banking", "Bank Unavailable")
        end

		showingAction = true
	end
end)

AddEventHandler("Polyzone:Exit", function(id, point, insideZone, data)
	if withinBranchZone and data and data.bank_branch then
		withinBranchZone = false
		if showingAction then
			plsr.Action:Hide("banking")
			showingAction = false
		end
	end
end)

AddEventHandler("Keybinds:Client:KeyUp:primary_action", function()
	if
		withinBranchZone
		and not plsr.State.flags.doingAction
		and not GlobalState[string.format("Fleeca:Disable:%s", withinBranchZone)]
	then
		TriggerEvent("Finance:Client:OpenUI", withinBranchZone)
	end
end)
